...
The maximum chunk size field takes in the number of bytes that the plugin will attempt to chunk data into before appending into the BigQuery Table. Fluent-Bit supports around a maximum of 2 MB of data within a single flush and we exercise a hard maximum of 9 MB (as BigQuery cannot handle appending data larger than this size). The maximum queue requests and maximum queue bytes fields describe the maximum number of requests/bytes of asynchrous responses that can be queued. Our plugin supports asynchronous request handling where responses are held in a queue without immediately attempting to check the response. If the responses aren't ready, they will be stored within the queue indefinetly - which is where these fields are relevant to configure the maximum "size" of this queue in terms of individual requests or bytes. When the first limit is reached, data appending will be blocked until enough responses are ready and the queue size decreases.  
